 ...
 *
 *   gcc -o siteinfo siteinfo.c -I../htslib ../htslib/libhts.a -lz -lpthread ../samtools/bedidx.o
	...
	while (bam_mplp_auto(mplp, &tid, &pos, n_plp, plp) > 0) { // come to the next covered position
		...
		for (i = 0; i < n; ++i) { // base level filters have to go here
			...
			for (j = 0; j < n_plp[i]; ++j) {
				...
				char isref = (ref[0] == translate_base(bam_seqi(bam_get_seq(p->b),p->qpos)) ? 'Y' : 'N');

				printf("%s\t%d\t%c\t%c\t%d\t%d\t%s\n",h->target_name[tid],pos+1,translate_base(bam_seqi(bam_get_seq(p->b),p->qpos)),
					isref,bam_get_qual(p->b)[p->qpos],((p->b)->core).qual,bam_get_qname(p->b));
